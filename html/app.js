    ...
    for (let i = 0; i < List.length; i++) {
        ...
        div.innerHTML = `<div class="w-9 h-9 rounded-full bg-purple-900 flex items-center justify-center text-purple-200 text-xs font-medium flex-shrink-0" ></div>
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-[0.65vw] text-zinc-400 m-0">Lejár: ${item.time === 0 ? 'Soha' : item.time}</p>
                    </div>
                    <span class=" text-[0.6vw] px-2 py-0.5 rounded-full ${item.time === 0 ? 'bg-red-900 text-red-300' : 'bg-green-900 text-green-300'}">${item.time === 0 ? 'Perm' : 'Temp'}</span>`
...
function selectBan(ban) {
    ...
    document.getElementById('banTime').textContent = isPerm ? 'Soha' : ban.time
...
document.getElementById('search').addEventListener('input', function (e) {
    ...
    if (filteredList.length === 0) {
        const container = document.getElementById('banList')
        container.innerHTML = '<p class="text-zinc-400 mt-5">Nincs találat</p>'
        return
    }
